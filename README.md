...
- `PUT /api/users/me/subscription-key` - Сохранение push-ключа подписок
- `POST /api/users/subscribe` - Подписка на другого пользователя
...
```json
{
  ...
  "subscription_key": "string (опционально, используется для push)"
}
```
...
- **Подписки и push-уведомления** (Celery + Redis + воркер)
...
│   ├── tasks/           # Celery конфигурация и задачи
│   ├── utils/           # Утилиты
...
- `backend` — API статей и постановка задач
- `users-api` — управление пользователями/подписками
- `redis` — брокер очереди для Celery
- `worker` — фоновые задачи рассылки
- `push-notificator` — локальный сервис для получения push-уведомлений
- `db-main` и `db-users` — отдельные PostgreSQL базы
- `api-gateway` — Nginx-прокси (маршрутизирует трафик)
...
## 🔔 Лабораторная №3 — подписки и фоновые задачи

В этой версии (ЛР3):

- добавлено хранение `subscription_key` и таблица `subscribers`
- реализованы эндпоинты `PUT /api/users/me/subscription-key` и `POST /api/users/subscribe`
- при создании статьи ставится Celery-задача в очередь Redis `article-notifications`
- отдельный сервис `worker` читает очередь, вытягивает подписчиков из Users DB и шлёт push-и в `push-notificator`
- таблица `notification_logs` гарантирует идемпотентность повторных задач

Чтобы протестировать:

1. Запустите `docker-compose up -d`.
2. Откройте `http://localhost:8000` (push-notificator) и скопируйте `subscription_key`.
3. Через `PUT /api/users/me/subscription-key` сохраните ключ.
4. Подпишитесь на автора (`POST /api/users/subscribe`).
5. Создайте статью через `/api/articles` — воркер автоматически отправит push подписчикам.

Подробное описание сценариев, очередей и воркера — в [LAB3_README.md](LAB3_README.md).

## 🔄 Лабораторная №4 — SAGA Choreography и публикация с модерацией

В этой версии (ЛР4):

- добавлены статусы постов: DRAFT, PENDING_PUBLISH, PUBLISHED, REJECTED, ERROR
- реализован эндпоинт `POST /api/articles/{slug}/publish` для запроса публикации
- двухэтапная публикация: модерация → генерация превью → публикация → уведомления
- реализован SAGA Choreography паттерн через цепочку асинхронных задач
- добавлена система внутренних API-ключей для service-to-service коммуникации
- настроен DLQ (Dead Letter Queue) с компенсирующими действиями
- все обработчики идемпотентны и устойчивы к сбоям

Чтобы протестировать:

1. Запустите `docker-compose up -d`.
2. Сгенерируйте API-ключи: `python scripts/generate_api_keys.py`.
3. Добавьте ключ в `.env`: `INTERNAL_API_KEY=<generated-key>`.
4. Создайте пост через `POST /api/articles` (статус: DRAFT).
5. Запросите публикацию через `POST /api/articles/{slug}/publish`.
6. Наблюдайте логи воркеров: `docker-compose logs -f saga-worker`.

Подробное описание архитектуры, задач и DLQ — в [LAB4_README.md](LAB4_README.md).
